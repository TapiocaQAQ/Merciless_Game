            ...
            for (int z = 0; z <= Dimension; z++)
            {
                ...
                for (int o = 0; o < octaves.Length; o++)
                {
                    if (octaves[o].alternate)
                    {
                        ...
                        y += Mathf.Cos(perl + octaves[o].speed.magnitude * Time.time) * octaves[o].height * (x + 5) / Dimension;
                    }
                    else
                    {
                ...
                var perl2 = Mathf.PerlinNoise((x * xzScale.x) / Dimension, (z * xzScale.y) / Dimension) * Mathf.PI * 2f;
                x2 += x + Mathf.Cos(perl2 + xzSpeed.magnitude * Time.time);

                verts[index(x, z)] = new Vector3(x2, y, z);
            }
        }
        Mesh.vertices = verts;
        Mesh.RecalculateNormals();
